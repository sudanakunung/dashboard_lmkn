	...
	<div class="card-header">
		<div class="row">
			<div class="col-md-4 pull-right">
				<?= form_search(base_url('likeadmininstitution/index'), 'keyword', 'Search Title or Artist'); ?>
			</div>
		</div>
	</div>
    ...
    if($dataLike) {
		...
		if($this->session->userdata('lembaga') == 'SELMI'){
			...
			foreach ($dataLike as $like){
				...
				if(!empty($like->lembaga_label)){
					if($like->lembaga_label <> 'SELMI'){
						$label = '';
					} else {
						$label = $like->label;
					}
				...
				isi_tabel_admin(++$start, array($like->title, $like->artist, $label, $like->arranger_name, $like->jumlah), "", "", $like->songId, false, false);
			}
		}
			...
			foreach ($dataLike as $like){
				...
				if(!empty($like->lembaga_composer)){
					if($like->lembaga_composer <> 'KCI'){
						$composer = '';
					} else {
				...
				isi_tabel_admin(++$start, array($like->title, $composer, $like->jumlah), "", "", $like->songId, false, false);
			}
		}
			...
			foreach ($dataLike as $like){
				...
				isi_tabel_admin(++$start, array($like->title, $like->artist, $label, $like->jumlah), "", "", $like->songId, false, false);
			}
		} else {
			...
			foreach ($dataLike as $like){
				...
				isi_tabel_admin(++$start, array($like->title, $like->artist, $like->label, $like->composer, $like->arranger_name, $like->jumlah), "", "", $like->songId, false, false);
			}
		}
