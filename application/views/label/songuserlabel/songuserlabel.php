<div class="row">
    ...
    <div class="col-md-offset-3 col-md-3 col-sm-12 col-xs-12">
        <form class="form-search" method="post" action="<?php echo base_url('songuserlabel/index')?>">
            <div class="input-group">
                <select class="form-control" name="tanggalsonguserlabel"  id="bulan" required="#ada">
...
<h3 class="page-header">
    <b>Song User</b>
    <a href="<?= base_url('songuserlabel/tambah') ?>" class="btn btn-primary btn-sm pull-right top-button"><i class="glyphicon glyphicon-plus-sign"></i> Add a Song</a>
</h3>
...
<div class="card mb-20" style="background: #eee;">
    <div class="card-header">
        <div class="row">
            <div class="col-md-4 pull-right">
                <?= form_search(base_url('songuserlabel/index'), 'keyword', 'Search Title'); ?>
            </div>
        </div>
    </div>
    ...
    <?php
    if($dataSongUser) {
       buka_tabel(array("Title", "Label", "Recording", "Like", "View", "Total"), $action = true, 'Chart');

        foreach ($dataSongUser as $songuser){
            
            $label_name = ($songuser->label_name == NULL || empty($songuser->label_name) ? "-" : $songuser->label_name);

            isi_tabel(++$start, array($songuser->title,$label_name,$songuser->JUMLAH_RECORDING, $songuser->JUMLAH_LIKE, $songuser->JUMLAH_VIEW, $songuser->jumlah), base_url('songuserlabel/grafik'),$songuser->songId,$awal,$akhir,true);
        }
