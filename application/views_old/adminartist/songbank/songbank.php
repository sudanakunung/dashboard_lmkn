...
<h3 class="page-header">
    <b>Song Bank</b>
    <a href="<?= base_url('songbankadminartist/tambah') ?>" class="btn btn-primary btn-sm pull-right top-button"><i class="glyphicon glyphicon-plus-sign"></i> Add a song </a>
</h3>
...
<div class="card mb-20">
    <div class="card-header">
        <div class="row">
            <div class="col-md-4 pull-right">
                <?= form_search(base_url('songbankadminartist/index'), 'keyword', 'Search Title'); ?>
            </div>
        </div>
    </div>
    ...
    <?php
    if($dataSongbank) {
       buka_tabel(array("Title", "Name","Recording", "Like", "View", "Total"), $action = true, 'Chart');

        foreach ($dataSongbank as $songbank){
            
            $artist_name = ($songbank->artist_name == NULL || empty($songbank->artist_name) ? "-" : $songbank->artist_name);

            isi_tabel(++$start, array($songbank->title,$artist_name,$songbank->JUMLAH_RECORDING, $songbank->JUMLAH_LIKE, $songbank->JUMLAH_VIEW, $songbank->jumlah), base_url('songbankadminartist/grafik'),$songbank->songId,$awal,$akhir,true);
        }
