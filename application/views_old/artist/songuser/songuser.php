<div class="row">
    ...
    <div class="col-md-offset-3 col-md-3 col-sm-12 col-xs-12">
        <form class="form-search" method="post" action="<?php echo base_url('songuser/index')?>">
...
<h3 class="page-header">
    <b>Song User</b>
    <a href="<?= base_url('songuser/tambah') ?>" class="btn btn-primary btn-sm pull-right top-button"><i class="glyphicon glyphicon-plus-sign"></i> Add a Song</a>
</h3>
...
<div class="card mb-20" style="background: #eee;">
    <div class="card-header">
        <div class="row">
            <div class="col-md-4 pull-right">
                <?= form_search(base_url('songuser/index'), 'keyword', 'Search Title'); ?>
            </div>
        </div>
    </div>
    ...
    <?php
    if($dataSongUser) {
       buka_tabel(array("Title", "Artist", "Recording", "Like", "View", "Total"), $action = true, 'Chart');

        foreach ($dataSongUser as $songuser){
            
            $artist_name = ($songuser->artist_name == NULL || empty($songuser->artist_name) ? "-" : $songuser->artist_name);

            isi_tabel(++$start, array($songuser->title,$artist_name,$songuser->JUMLAH_RECORDING, $songuser->JUMLAH_LIKE, $songuser->JUMLAH_VIEW, $songuser->jumlah), base_url('songuser/grafik'),$songuser->songId,$awal,$akhir,true);
        }
