...
<div class="card mb-20">
  <div class="card-header">
    <div class="row">
      <div class="col-md-4 pull-right">
        <?= form_search(base_url('songrequestadmin/index'), 'keyword', 'Search Title'); ?>
      </div>
    </div>
  </div>
  ...
  <?php
  if($dataSongRequest) {
    buka_tabel(array("Song Id", "Title","Requested by", "Update Data"), $no_action = false);
    ...
    foreach ($dataSongRequest as $request){
      ...
      else if ($request->arrangerId > 0){
        $nameRequestedBy = "Arranger ".$request->arranger;
      } else {
        $nameRequestedBy = "Composer ".$request->composer;
      }

      isi_tabel_request(++$start, array($request->songId, $request->title, $nameRequestedBy),base_url("songrequestadmin/aksiapprove"), base_url("songrequestadmin/aksidecline"), $request->tsongrequestId, true, true);  
    }
