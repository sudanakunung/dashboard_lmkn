    ...
    <div class="card-header">
        <div class="row">
            <div class="col-md-4 pull-right">
                <?= form_search(base_url('songuserarranger/tambah'), 'keyword', 'Search Title or Artist'); ?>
            </div>
        </div>
    </div>
    ...
    if($dataSongUserToAdd) {
       buka_tabel(array("ID", "Name", "Artist"), $action = true, "Add to Request");
        ...
        foreach ($dataSongUserToAdd as $lagu){
            ...
            isi_tabel_admin(++$start, array($lagu->songId,$lagu->title, $lagu->artistName), "", "", $lagu->songId, false, false, '<a href="'.base_url('songuserarranger/aksitambah').'/'.$this->session->userdata('arrangerId').'/'.$lagu->songId.'" class="btn btn-primary"><span class="glyphicon glyphicon-plus"></span> Add to Request</a>');
        }
