...
class Songadmin extends Admin_Controller {

    public function __construct()
    {
        parent::__construct();
        $this->halaman = "songadmin";
    }
    ...
    public function index()
    {
        ...
        if ( $keyword != Null && $musiclangcode != Null ) {
            ...
            $url = base_url('songadmin/index').'?keyword='.$keyword.'&musiclangcode='.$musiclangcode;
        }
        else if($keyword != Null ||$musiclangcode != Null){
            if($keyword != Null){
                $keyword = $this->input->get('keyword', true);
                $url = base_url('songadmin/index').'?keyword='.$keyword;
            }

            if($musiclangcode != Null){
                $musiclangcode = $this->input->get('musiclangcode', true);
                $url = base_url('songadmin/index').'?musiclangcode='.$musiclangcode;
            }
        } 
        else {
            $keyword = Null;
            $musiclangcode = Null;
            $url = base_url('songadmin/index');
        }

        $config['total_rows'] = $this->songadmin->countDataSong($keyword, $musiclangcode);
        ...
        $dataLang  = $this->songadmin->getLang();
        $dataSong  = $this->songadmin->getDataSong($offset, $dataPerPage, $keyword, $musiclangcode);

        $mainview   = "admin/song/default";
    ...
    {
        $dataEditSong = $this->songadmin->getDataEditSong($songId);
        $dataArtist = $this->songadmin->getDataArtist();
        $dataLabel = $this->songadmin->getDataLabel();
        $dataArranger = $this->songadmin->getDataArranger();
        $dataComposer = $this->songadmin->getDataComposer();
        $mainview = "admin/song/edit";
        $halaman  = $this->halaman;
    ...
    public function aksiedit()
    {
        ...
        if($this->input->post("aksi") == "edit") {
            ...
            if (empty($_FILES['cover']['name'])) {
                ...
                $aksi           = $this->songadmin->editsong($judul, $artistId, $recordLabelId, $arrangerId, $composerId, $description, $namafileText, $songId);

                if($aksi == true){
                    $this->session->set_flashdata('sukses_aksi_editlagu', '<div class="alert alert-success" role="alert">Data successfully updated</div>');
                    redirect(base_url('songadmin/edit/'.$songId));
                }
                else{
                ...
                if ($this->upload->do_upload('cover')) {
                    ...
                    $getOldImage = $this->songadmin->getOldImage($songId);
                    ...
                    $aksi = $this->songadmin->editsong($judul, $artistId, $recordLabelId, $arrangerId, $composerId, $description, $namaFileAfterUpload, $songId);
