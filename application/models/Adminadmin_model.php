...
class Adminadmin_model extends MY_Model{
    ...
    public function getDataEdit($adminId){
        $sql = "SELECT adminId, username, artistId, email, roleId, recordLabelId, composerId, arrangerId, lembaga, langCode FROM tadmin WHERE adminId = ?";
        ...
        if($datas->artistId > 0){
            $data = array("adminId" => $datas->adminId, "username" => $datas->username, "artistId"=> $datas->artistId,"email"=>$datas->email, "password"=>"","roleId"=>0,"recordLabelId"=>$datas->recordLabelId, "arrangerId"=>$datas->arrangerId, "composerId"=>$datas->composerId, "lembaga"=>$datas->lembaga, "langCode"=>$datas->langCode);
        }
        elseif($datas->recordLabelId > 0){
            $data = array("adminId" => $datas->adminId, "username" => $datas->username, "artistId"=> $datas->artistId,"email"=>$datas->email, "password"=>"","roleId"=>2,"recordLabelId"=>$datas->recordLabelId, "arrangerId"=>$datas->arrangerId, "composerId"=>$datas->composerId, "lembaga"=>$datas->lembaga, "langCode"=>$datas->langCode);
        }
        elseif($datas->composerId > 0){
            $data = array("adminId" => $datas->adminId, "username" => $datas->username, "artistId"=> $datas->artistId,"email"=>$datas->email, "password"=>"","roleId"=>3,"recordLabelId"=>$datas->recordLabelId, "arrangerId"=>$datas->arrangerId, "composerId"=>$datas->composerId, "lembaga"=>$datas->lembaga, "langCode"=>$datas->langCode);
        }
        elseif($datas->arrangerId > 0){
            $data = array("adminId" => $datas->adminId, "username" => $datas->username, "artistId"=> $datas->artistId,"email"=>$datas->email, "password"=>"","roleId"=>7,"recordLabelId"=>$datas->recordLabelId, "arrangerId"=>$datas->arrangerId, "composerId"=>$datas->composerId, "lembaga"=>$datas->lembaga, "langCode"=>$datas->langCode);
        }
        else{
            $data = array("adminId" => $datas->adminId, "username" => $datas->username, "artistId"=> $datas->artistId,"email"=>$datas->email, "password"=>"","roleId"=>$datas->roleId,"recordLabelId"=>$datas->recordLabelId, "arrangerId"=>$datas->arrangerId, "composerId"=>$datas->composerId, "lembaga"=>$datas->lembaga, "langCode"=>$datas->langCode);
        }
    ...
    public function editAdmin($username,$artistId,$email,$hasing,$roleId,$recordLabelId,$arrangerId,$adminId,$composerId,$lembaga){
        ...
        if($this->session->userdata('admincountry') == '' || ($this->session->userdata('roleId') == '1' && $this->session->userdata('admincountry') == 'ind')){
